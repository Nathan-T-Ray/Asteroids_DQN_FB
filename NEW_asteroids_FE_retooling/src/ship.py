...
class Ship(Shooter):
    ...
    #The first of many methods that will be used to judge game state
    #NATHANCODE
    ...
    def __str__(self):
        return f"Ship(position=({self.position.x:.2f}, {self.position.y:.2f}), heading=({self.heading.x:.2f}, {self.heading.y:.2f}), angle={self.angle}, visible={self.visible}, inHyperSpace={self.inHyperSpace})"
