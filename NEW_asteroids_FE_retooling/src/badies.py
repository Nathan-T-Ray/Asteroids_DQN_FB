...
class Rock(VectorSprite):
    ...
    def __str__(self):
        return f"Rock(position=({self.position.x:.2f}, {self.position.y:.2f}), heading=({self.heading.x:.2f}, {self.heading.y:.2f}), rockType={self.rockType})"    
...
class Debris(Point):    
    ...
    def __str__(self):
        return f"Debris({self.position.x:.2f}, {self.position.y:.2f}), heading=({self.heading.x:.2f}, {self.heading.y:.2f}), ttl={self.ttl})"        
...
class Saucer(Shooter):
    ...
    def fireBullet(self):
        if self.ship is not None:            
            ...
            shotFired = Shooter.fireBullet(self, heading, self.bulletTtl[self.saucerType], self.bulletVelocity, False)
            #if shotFired:
                #playSound("sfire")

    
    def __str__ (self):
        return f"Saucer(position=({self.position.x:.2f}, {self.position.y:.2f}), heading=({self.heading.x:.2f}, {self.heading.y:.2f}), saucerType={self.saucerType})"